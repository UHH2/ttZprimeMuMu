...
ttZPrimeTTBarGenStudyHists::ttZPrimeTTBarGenStudyHists(uhh2::Context & ctx, const std::string & dirname, const std::string & hyps_name, const std::string & discriminator_name_reco,  const std::string & discriminator_name_match ): Hists(ctx, dirname){
    ...
    matchedbjets1 = book<TH1F> ("RightRecoBJets1","RightRecoBJets1",5,0,4);
    matchedbjets2 = book<TH1F> ("RightRecoBJets2","RightRecoBJets2",5,0,4);
    matchedjets = book<TH1F> ("RightRecoJets","RightRecoJets",5,0,4);
    matchedjets1 = book<TH1F> ("RightRecoJets1","RightRecoJets1",5,0,4);
    matchedjets2 = book<TH1F> ("RightRecoJets2","RightRecoJets2",5,0,4);
    matchedw = book<TH1F> ("RightRecoW","RightRecoW",5,0,4);
    matchedw1 = book<TH1F> ("RightRecoW1","RightRecoW1",5,0,4);
    matchedw2 = book<TH1F> ("RightRecoW2","RightRecoW2",5,0,4);
...
void ttZPrimeTTBarGenStudyHists::fill(const uhh2::Event & e){
  ...
  // auto  jets = e.jets;
  jets = hyp_reco->tophad1_jets();
  jets.insert(jets.end(),hyp_reco->tophad2_jets().begin(), hyp_reco->tophad2_jets().end());
  jets.insert(jets.end(),hyp_reco->tophad1_bjet());
  jets.insert(jets.end(),hyp_reco->tophad2_bjet());
  ...
  if (jets1_match.size() == jets1_reco.size())
  {
    ...
    {
      if(deltaR(jets1_match.at(i),jets1_reco.at(i))< 0.4)
      {
       matchedjets->Fill(1, weight);
       matchedjets1->Fill(1, weight);
      }
      else
      {
        matchedjets->Fill(3, weight);
        matchedjets1->Fill(3, weight);
      }
    }
  }
  if (jets2_match.size() == jets2_reco.size())
  {
    ...
    {
      if(deltaR(jets2_match.at(i),jets2_reco.at(i))< 0.4)
      {
        matchedjets->Fill(1, weight);
        matchedjets2->Fill(1, weight);
      }
      else
      {
        matchedjets->Fill(3, weight);
        matchedjets2->Fill(3, weight);
      }
    }
  }
  if(deltaR(hyp_match->tophad1_bjet(),hyp_reco->tophad1_bjet())< 0.4)
  {
    matchedbjets->Fill(1, weight);
    matchedbjets1->Fill(1, weight);
  }
  else
  {
    matchedbjets->Fill(3, weight);
    matchedbjets1->Fill(3, weight);
  }
  if(deltaR(hyp_match->tophad2_bjet(),hyp_reco->tophad2_bjet())< 0.4)
  {
    matchedbjets->Fill(1, weight);
    matchedbjets2->Fill(1, weight);
  }
  else
  {
    matchedbjets->Fill(3, weight);
    matchedbjets2->Fill(3, weight);
  }
  if(deltaR(hyp_match->w1_v4(),hyp_reco->w1_v4())< 0.4)
  {
    matchedw->Fill(1, weight);
    matchedw1->Fill(1, weight);
  }
  else
  {
    matchedw->Fill(3, weight);
    matchedw1->Fill(3, weight);
  }
  if(deltaR(hyp_match->w2_v4(),hyp_reco->w2_v4())< 0.4)
  {
    matchedw->Fill(1, weight);
    matchedw2->Fill(1, weight);
  }
  else
  {
    matchedw->Fill(3, weight);
    matchedw2->Fill(3, weight);
  }
